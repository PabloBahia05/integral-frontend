...
const applyRender = (col) => {
  ...
  if (col.key.startsWith("valor")) {
    return {
      ...col,
      render: (v) => (v != null && v !== "" ? formatPeso(v) : "—"),
    };
  }
  if (col.key === "REVISION") {
    return {
  ...
  if (col.key === "presp") {
    // El backend genera presp con formato propio (ej: "P00008");
    // si viniera como número plano, lo dejamos igual (sin padding forzado).
    return { ...col, render: (v) => (v ? String(v) : "—") };
  }
  return col;
};

export default function PresupuestosPuertasTabla({
  presupuestos,
  onSave,
  onDelete,
  selected,
  ...
  const filtered = rows.filter((p) => {
    ...
    return (
      ...
      String(p.presp ?? p.id ?? "").includes(q)
    );
  });
  ...
  const openEdit = () => {
    ...
    setForm({
      ...
      ALTO: selected.ALTO ?? "",
      VIDRIO: selected.VIDRIO ?? "esmerilado",
  ...
  const openHistorial = async () => {
    ...
    try {
      const numeroParam = selected.presp ?? selected.id;
      const res = await authFetch(
        `${API}/presupuestos-puertas/${numeroParam}/revisiones`,
      );
  ...
  const handleSubmit = () => {
    ...
    for (let n = 1; n <= 10; n++) {
      ...
      artVals[`valor${n}`] =
        form[`valor${n}`] !== "" && form[`valor${n}`] != null
          ? Number(form[`valor${n}`])
          : null;
      artVals[`margen${n}`] =
        form[`margen${n}`] !== "" && form[`margen${n}`] != null
          ? Number(form[`margen${n}`])
          : null;
    }
    const payload = {
      NOMBRE: form.NOMBRE,
    ...
    onSave(
      modal === "nuevo"
        ? payload
        : { ...payload, presp: selected.presp ?? selected.id },
    );
    onCloseModal();
    setForm(EMPTY);
    setPrecioCalc(null);
  ...
  const buildColumns = (dataRows) => {
    ...
    const artCols = [];
    for (let n = 1; n <= 10; n++) {
      const hasArt = dataRows.some(
        (r) => r["art" + n] != null && r["art" + n] !== "",
      );
      if (!hasArt) continue;
      // Label = nombre del artículo (tomado de la primera fila que lo tenga)
      const artLabel =
        (dataRows.find((r) => r["art" + n]) || {})["art" + n] || "Art. " + n;
  ...
  const totalPresupuestado = rows.reduce(
  ...
  return (
    <>
      <style>{`
        ...
        .pp-form-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 0 24px; }
        .pp-form-group { margin-bottom: 14px; }
        .pp-form-group label { display: block; font-size: 11px; font-weight: 700; letter-spacing: 0.12em; color: #4a8ab5; text-transform: uppercase; margin-bottom: 5px; }
        .pp-vidrio-row { display: flex; gap: 8px; }
        .pp-vidrio-btn { flex: 1; padding: 9px; border-radius: 5px; border: 1.5px solid #d0dde8; background: #fff; cursor: pointer; font-size: 13px; font-weight: 600; color: #4a6a80; transition: all 0.15s; }
        .pp-vidrio-btn.active { background: #0f2944; border-color: #0f2944; color: #fff; }
      ...
      <ScreenHeader
        icon="🚪"
        title="Presupuestos Puertas"
        subtitle="Historial y gestión de presupuestos"
      />
      ...
      <StatCards
        stats={[
          ...
          { label: "Filtrados", value: filtered.length },
          {
            ...
            value: formatPeso(totalPresupuestado),
          },
        ]}
      />
      ...
      {(modal === "nuevo" || modal === "editar") && (
        <Modal
          title={
            modal === "nuevo"
              ? "Nuevo presupuesto"
              : `Editar presupuesto N° ${String(selected?.presp ?? selected?.id ?? "").padStart(4, "0")} — guardará nueva revisión`
          }
        ...
        >
          ...
          <div className="pp-form-grid">
            ...
            <div>
              ...
              <div className="pp-form-group">
                ...
                <div className="pp-vidrio-row">
                  {["incoloro", "esmerilado"].map((v) => (
                    <button
                      key={v}
                      className={`pp-vidrio-btn${form.VIDRIO === v ? " active" : ""}`}
            ...
            <div>
              ...
              <div className="pp-form-group">
                ...
                {precioCalc !== null &&
                  Number(precioCalc) !== Number(form.PRECIO) && (
                    <div
                      style={{
                        fontSize: "10px",
                        color: "#856404",
          ...
          {modal === "editar" && (
            ...
            >
              ⚠️ Al actualizar se guardará una nueva revisión incremental. Las
      ...
      {modalHistorial &&
        (() => {
          ...
          const revArtSlots = [];
          for (let n = 1; n <= 10; n++) {
            const hasArt = revisiones.some(
              (r) => r[`art${n}`] != null && r[`art${n}`] !== "",
            );
            if (!hasArt) continue;
            const label =
              (revisiones.find((r) => r[`art${n}`]) || {})[`art${n}`] ||
              `Art. ${n}`;
            const hasMargen = revisiones.some(
              (r) =>
                r[`margen${n}`] != null &&
                r[`margen${n}`] !== "" &&
                Number(r[`margen${n}`]) !== 0,
          ...
          return (
            <Modal
              title={`Historial de revisiones — N° ${String(selected?.presp ?? selected?.id ?? "").padStart(4, "0")} · ${selected?.NOMBRE ?? ""}`}
            ...
            >
              {loadingRev ? (
                ...
                <p className="rev-empty">
                  No hay revisiones registradas para este presupuesto.
                </p>
              ) : (
                  ...
                  <tbody>
                    {revisiones.map((r) => (
                      <tr key={r.id}>
                        <td>
                          <span
                            className={`rev-badge ${Number(r.REVISION) === 0 ? "rev-badge-0" : "rev-badge-n"}`}
                          >
                            Rev. {r.REVISION}
                          </span>
                        </td>
                        ...
                        {revArtSlots.map(({ n, hasMargen }) => (
                          <>
                            <td key={`val${n}`}>
                              {r[`valor${n}`] != null && r[`valor${n}`] !== ""
                                ? formatPeso(r[`valor${n}`])
                                : "—"}
                            </td>
                            {hasMargen && (
                              <td key={`mar${n}`}>
                        ...
                        <td style={{ fontWeight: 700, color: "#0f2944" }}>
                          {formatPeso(r.PRECIO)}
                        </td>
                      </tr>
                    ))}
