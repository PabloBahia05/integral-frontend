...
export default function FormulasProduccion({
  ...
  const verificarFormula = async () => {
    ...
    try {
      const res = await authFetch(
        "https://integral-backend-production.up.railway.app/formulas-produccion/verificar",
        {
          method: "POST",
  ...
  const cargarCascadaDeArticulo = async (codVinculado, rubroFallback) => {
    if (codVinculado) {
      try {
        const res = await authFetch(
        ...
        if (data.familia) setFamiliaElegida(data.familia);
      } catch {
        /* silencioso */
      }
    } else if (rubroFallback) {
      try {
        const resFam = await authFetch(
          `https://integral-backend-production.up.railway.app/articulos/familias-por-rubro?rubro=${encodeURIComponent(rubroFallback)}`,
        );
        const fams = await resFam.json();
        if (Array.isArray(fams) && fams.length > 0) setFamilias(fams);
      } catch {
  ...
  const handleSubmit = async () => {
    ...
    // La tabla FORMULAS_PRODUCCION también usa la columna "codart" — mapeamos antes de enviar
    ...
    if (modal === "nuevo") {
      try {
        const res = await authFetch(
          "https://integral-backend-production.up.railway.app/formulas-produccion",
          {
            method: "POST",
            headers: { "Content-Type": "application/json" },
  ...
  return (
    <>
      <ScreenHeader
        ...
        subtitle="Gestión de fórmulas de cálculo para Producción"
      />
      ...
      {(modal === "nuevo" || modal === "editar") && (
        <Modal
          title={modal === "nuevo" ? "Nueva fórmula de producción" : "Editar fórmula de producción"}
        ...
        >
          ...
          <div className="form-group">
            ...
            <input
              className="form-input"
              placeholder="Ej: FPRODCORTE, FPRODTIEMPO"
              value={form.codform}
              onChange={(e) =>
          ...
          <div className="form-group">
            ...
            <input
              className="form-input"
              placeholder="Ej: Cantidad de tornillos, Tiempo de armado..."
