...
function InsertarMargen({ formulaRef, setForm, codart, authFetch }) {
...
export default function FormStd({
  formulas,
  onSave,
  onDelete,
  selected,
  ...
  const verificarFormula = async () => {
    ...
    try {
      const res = await authFetch(
        "https://integral-backend-production.up.railway.app/form-std/verificar",
        {
          method: "POST",
  ...
  const handleSubmit = async () => {
    ...
    // La tabla FORM_STD sigue usando la columna "codart" — mapeamos antes de enviar
    ...
    if (modal === "nuevo") {
      try {
        const res = await authFetch(
          "https://integral-backend-production.up.railway.app/form-std",
          {
            method: "POST",
            headers: { "Content-Type": "application/json" },
  ...
  return (
    <>
      <ScreenHeader
        icon="🧮"
        title="Fórmulas Estándar"
        subtitle="Gestión de fórmulas estándar de cálculo"
      />
      ...
      {(modal === "nuevo" || modal === "editar") && (
        <Modal
          title={modal === "nuevo" ? "Nueva fórmula estándar" : "Editar fórmula estándar"}
