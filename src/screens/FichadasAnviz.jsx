...
function parseMysqlTs(ts) {
  if (!ts) return null;
  // "2024-05-10 08:32:00" → tratar como UTC
...
export default function FichadasAnviz() {
  ...
  }, [filtroFecha, filtroUser]);

  useEffect(() => { cargarFichadas(); }, [cargarFichadas]);
  ...
  useEffect(() => {
    if (!API) return;
    const wsUrl = API.replace(/^http/, "ws") + "/ws"; // ajustar si la ruta WS es distinta
    try {
      ...
      ws.onmessage = (e) => {
        try {
          ...
          if (msg.type === "fichada") {
            ...
            // Solo agregar si coincide con el filtro de fecha activo
            const fechaNueva = nueva.timestamp?.slice(0, 10);
            if (!filtroFecha || fechaNueva === filtroFecha) {
  ...
  }, [filtroFecha]); // reconectar si cambia filtro de fecha

  // ── Derivados ──────────────────────────────────────────────────────────────
  const fichadasFiltradas = fichadas.filter(f => {
    if (filtroDireccion !== "todas" && f.direccion !== filtroDireccion) return false;
    if (busquedaNombre) {
      const nombre = (f.nombre_completo || "").toLowerCase();
      if (!nombre.includes(busquedaNombre.toLowerCase())) return false;
    }
    return true;
  });
    ...
    <div style={s.page}>
      ...
      <div style={s.header}>
        ...
        <div style={s.headerRight}>
          ...
          <button style={s.btnIcon} onClick={cargarFichadas} title="Actualizar" disabled={cargando}>
      ...
      <div style={s.filtrosWrap}>
        <div style={s.filtroFila}>
          ...
          {/* Búsqueda nombre libre */}
          <div style={s.filtroGrupo}>
            <label style={s.label}>Buscar nombre</label>
            <input type="text" placeholder="Apellido o nombre..."
        ...
        {/* Tabs vista */}
        <div style={s.tabs}>
          <button style={vista === "tabla"   ? s.tabActive : s.tab} onClick={() => setVista("tabla")}>
            Detalle
          </button>
          <button style={vista === "resumen" ? s.tabActive : s.tab} onClick={() => setVista("resumen")}>
            Por empleado
          </button>
        </div>
      </div>
      ...
      <div style={s.tableWrap}>
        ...
        {cargando ? (
          <div style={s.estado}>
            <div style={s.spinner} />
            <span style={{ color: "#64748b", fontSize: 14 }}>Cargando registros...</span>
          </div>
              ...
              <tbody>
                {visibles.map((f, i) => (
                  <tr key={f.id ?? i} style={s.trHover}>
                    <td style={{ ...s.td, color: "#475569", fontSize: 12, width: 40 }}>{inicio + i + 1}</td>
                    <td style={s.td}>
                      <div style={s.empNombre}>
                        <span style={s.empAvatar}>
                          {(f.nombre_completo?.trim() || `U${f.user_id}`).charAt(0).toUpperCase()}
                        </span>
                        <div>
                          <div style={{ fontWeight: 500, fontSize: 13 }}>
                            {f.nombre_completo?.trim() || `Usuario ${f.user_id}`}
                          </div>
                          <div style={{ fontSize: 11, color: "#64748b" }}>ID {f.user_id}</div>
                        </div>
                      </div>
                    </td>
            ...
            <tbody>
              {resumenEmpleados.map(emp => (
                <tr key={emp.user_id} style={s.trHover}>
                  <td style={s.td}>
                    <div style={s.empNombre}>
                      <span style={s.empAvatar}>{emp.nombre.charAt(0).toUpperCase()}</span>
                      <div>
                  ...
                  <td style={s.td}>
                    <span style={emp.salidas > 0 ? s.badgeSalida : s.badgeGray}>↑ {emp.salidas}</span>
                  </td>
...
function StatCard({ label, valor, color }) {
  return (
    <div style={{ ...s.card, borderLeft: `3px solid ${color}` }}>
      ...
      <div style={{ fontSize: 30, fontWeight: 700, color: "#f1f5f9", fontVariantNumeric: "tabular-nums" }}>{valor}</div>
    </div>
  );
}
...
// ─── Estilos ──────────────────────────────────────────────────────────────────
const s = {
  page:        { minHeight: "100vh", background: "#0f172a", padding: "28px 32px", fontFamily: "'DM Sans','Helvetica Neue',sans-serif", color: "#f1f5f9" },
  header:      { display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 28 },
  headerLeft:  { display: "flex", alignItems: "center", gap: 14 },
  headerRight: { display: "flex", alignItems: "center", gap: 10 },
  iconBox:     { width: 44, height: 44, background: "#1e293b", border: "1px solid #334155", borderRadius: 10, display: "flex", alignItems: "center", justifyContent: "center" },
  titulo:      { margin: 0, fontSize: 21, fontWeight: 700, color: "#f1f5f9" },
  subtitulo:   { fontSize: 12, color: "#475569" },
  btnIcon:     { background: "#1e293b", border: "1px solid #334155", borderRadius: 8, padding: "8px 10px", cursor: "pointer", color: "#94a3b8", display: "flex", alignItems: "center" },

  cards:       { display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(130px, 1fr))", gap: 12, marginBottom: 24 },
  card:        { background: "#1e293b", border: "1px solid #1e293b", borderRadius: 12, padding: "16px 20px" },

  filtrosWrap: { background: "#1e293b", border: "1px solid #334155", borderRadius: 12, padding: "16px 20px", marginBottom: 20 },
  filtroFila:  { display: "flex", flexWrap: "wrap", gap: 12, alignItems: "flex-end", marginBottom: 14 },
  filtroGrupo: { display: "flex", flexDirection: "column", gap: 4 },
  label:       { fontSize: 11, color: "#475569", textTransform: "uppercase", letterSpacing: "0.06em" },
  input:       { background: "#0f172a", border: "1px solid #334155", borderRadius: 7, padding: "7px 11px", color: "#f1f5f9", fontSize: 13, outline: "none", minWidth: 150 },
  btnBuscar:   { background: "#6366f1", border: "none", borderRadius: 7, padding: "8px 22px", color: "#fff", fontWeight: 600, fontSize: 13, cursor: "pointer", alignSelf: "flex-end" },

  tabs:        { display: "flex", gap: 4 },
  tab:         { background: "transparent", border: "1px solid #334155", borderRadius: 7, padding: "5px 16px", color: "#64748b", fontSize: 13, cursor: "pointer" },
  tabActive:   { background: "#312e81", border: "1px solid #6366f1", borderRadius: 7, padding: "5px 16px", color: "#a5b4fc", fontSize: 13, cursor: "pointer", fontWeight: 600 },
  ...
  estado:      { display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 10, padding: "60px 0" },
  spinner:     { width: 30, height: 30, border: "3px solid #334155", borderTop: "3px solid #6366f1", borderRadius: "50%", animation: "spin 0.8s linear infinite" },

  tabla:       { width: "100%", borderCollapse: "collapse" },
  th:          { padding: "11px 18px", textAlign: "left", fontSize: 11, color: "#475569", textTransform: "uppercase", letterSpacing: "0.07em", borderBottom: "1px solid #334155", background: "#0f172a50" },
  td:          { padding: "12px 18px", fontSize: 13, color: "#cbd5e1", borderBottom: "1px solid #1e293b" },
  trHover:     {},

  empNombre:   { display: "flex", alignItems: "center", gap: 10 },
  empAvatar:   { width: 30, height: 30, borderRadius: "50%", background: "#312e81", color: "#a5b4fc", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, fontWeight: 700, flexShrink: 0 },

  badgeEntrada:{ background: "#052e16", color: "#4ade80", border: "1px solid #16a34a44", borderRadius: 99, padding: "3px 10px", fontSize: 12, fontWeight: 500, whiteSpace: "nowrap" },
  badgeSalida: { background: "#2d1b00", color: "#fbbf24", border: "1px solid #d9770644", borderRadius: 99, padding: "3px 10px", fontSize: 12, fontWeight: 500, whiteSpace: "nowrap" },
  badgeGray:   { background: "#1e293b", color: "#64748b", border: "1px solid #33415544", borderRadius: 99, padding: "3px 10px", fontSize: 12, fontWeight: 500, whiteSpace: "nowrap" },

  paginacion:  { display: "flex", justifyContent: "space-between", alignItems: "center", padding: "14px 18px", borderTop: "1px solid #334155" },
  btnPag:      { background: "#0f172a", border: "1px solid #334155", borderRadius: 7, padding: "6px 14px", color: "#94a3b8", fontSize: 13, cursor: "pointer" },
};
