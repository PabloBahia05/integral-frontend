...
const EMPTY = {
  ancho: "100", alto: "50", profundidad: "50",
  ...
  // Cajones
  cajon1_ancho: "0", cajon1_alto: "0", cajon1_cantidad: "0",
  cajon2_ancho: "0", cajon2_alto: "0", cajon2_cantidad: "0",
  cajon3_ancho: "0", cajon3_alto: "0", cajon3_cantidad: "0",
  // Puertas
  puerta1_ancho: "0", puerta1_alto: "0", puerta1_cantidad: "0",
  puerta2_ancho: "0", puerta2_alto: "0", puerta2_cantidad: "0",
  puerta3_ancho: "0", puerta3_alto: "0", puerta3_cantidad: "0",
  // Mano de obra por fórmula (slot 1..10)
  mo_form1: 0, mo_form2: 0, mo_form3: 0, mo_form4: 0, mo_form5: 0,
  mo_form6: 0, mo_form7: 0, mo_form8: 0, mo_form9: 0, mo_form10: 0,
  // Expresión de mano de obra (texto libre evaluable)
  mo_expr1: "", mo_expr2: "", mo_expr3: "", mo_expr4: "", mo_expr5: "",
  mo_expr6: "", mo_expr7: "", mo_expr8: "", mo_expr9: "", mo_expr10: "",
};
...
function PuertaSection({ num, form, setForm }) {
  const set = (key) => (val) => setForm(f => ({ ...f, [`puerta${num}_${key}`]: val }));
  const filled = form[`puerta${num}_ancho`] || form[`puerta${num}_alto`] || form[`puerta${num}_cantidad`];
  return (
    <div style={{
      background: filled ? "#f0f7ff" : "#f8fafc",
      border: `1.5px solid ${filled ? "#93c5fd" : "#e2e8f0"}`,
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12 }}>
        <Field label="Ancho"    value={form[`puerta${num}_ancho`]}    onChange={set("ancho")} />
        <Field label="Alto"     value={form[`puerta${num}_alto`]}     onChange={set("alto")} />
        <Field label="Cantidad" value={form[`puerta${num}_cantidad`]} onChange={set("cantidad")} unit="u." integer />
      </div>
    </div>
  );
}
...
export default function MuebleEspecial() {
      ...
      .then(d => {
        ...
        for (const k of Object.keys(row)) {
          if (/margen|margin/i.test(k)) {
            ...
            if (!isNaN(v)) { raw = v; break; }
          }
        }
        if (isNaN(raw)) { setMargenBD(null); return; }
  ...
  useEffect(() => {
    fetch(`${API}/colocacion/buscar?codart=${encodeURIComponent("ESPECIAL")}`)
      ...
      .then(d => {
        const row = Array.isArray(d) ? d[0] : d;
        if (!row) { setColocacionBD(null); return; }
        const val = parseFloat(row.precio ?? row.PRECIO ?? row.valor ?? row.VALOR ?? NaN);
        const pct = parseFloat(row.porcentaje ?? row.PORCENTAJE ?? row.pct ?? NaN);
        const bd  = {
        ...
        if (bd.valor !== null)      { setColocacionModo("valor");      setColocacionValor(bd.valor); }
        else if (bd.porcentaje !== null) { setColocacionModo("porcentaje"); setColocacionPct(bd.porcentaje); }
      })
  ...
  const makeVars = (f) => ({
    ...
    precio_lateral_izq: f.lat_izq === "COLOR"  ? (Number(f.materialPrecio)       || 0)
                       : f.lat_izq === "BLANCO" ? (Number(f.materialBlancoPrecio) || 0) : 0,
    precio_lateral_der: f.lat_der === "COLOR"  ? (Number(f.materialPrecio)       || 0)
                       : f.lat_der === "BLANCO" ? (Number(f.materialBlancoPrecio) || 0) : 0,
    precio_techo:       f.techo === "COLOR"  ? (Number(f.materialPrecio)       || 0)
                       : f.techo === "BLANCO" ? (Number(f.materialBlancoPrecio) || 0) : 0,
    precio_base:        f.piso  === "COLOR"  ? (Number(f.materialPrecio)       || 0)
                       : f.piso  === "BLANCO" ? (Number(f.materialBlancoPrecio) || 0) : 0,
    // Cajones
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetch(`${API}/asociaciones-form?codart=${encodeURIComponent(CODART_ESP)}`).then(r => r.json()).catch(() => null),
      ...
      const row = todos.find(a =>
        (a.codart ?? a.CODART ?? "").toUpperCase() === CODART_ESP.toUpperCase()
      ) ?? null;
      ...
      await Promise.all(
        codartsBD.map(async (cod) => {
          try {
            ...
            let precio = NaN;
            for (const campo of ['PRECIO1','precio1','PRECIO','precio','PREC1','prec1','COSTO','costo','VALOR','valor']) {
  ...
  useEffect(() => {
    ...
    const vars = makeVars(form);
    const actualizados = slotsFormulas.map(s => ({ ...s, resultado: evalExpr(s.expresion, vars) }));
    const suma = actualizados.reduce((a, s) => a + (s.resultado ?? 0), 0);
    setSlotsFormulas(actualizados);
    setTotalSlots(Math.round(suma * 100) / 100);
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [
    form.ancho, form.alto, form.profundidad,
    form.materialPrecio, form.materialBlancoPrecio,
    form.lat_izq, form.lat_der, form.techo, form.piso,
    form.cajon1_ancho, form.cajon1_alto, form.cajon1_cantidad,
    form.cajon2_ancho, form.cajon2_alto, form.cajon2_cantidad,
    form.cajon3_ancho, form.cajon3_alto, form.cajon3_cantidad,
    form.puerta1_ancho, form.puerta1_alto, form.puerta1_cantidad,
    form.puerta2_ancho, form.puerta2_alto, form.puerta2_cantidad,
    form.puerta3_ancho, form.puerta3_alto, form.puerta3_cantidad,
  ]);
    ...
    for (let i = 1; i <= 10; i++) {
      ...
      if (!/^[\d.]+$/.test(e)) {
        try {
          ...
          if (res !== form[moKey]) { updates[moKey] = res; changed = true; }
        } catch { /* expr inválida, no actualizar */ }
      }
    }
  ...
  const totalColocacion = colocacionModo === "porcentaje"
  ...
  return (
    <div style={{ maxWidth: 680, margin: "0 auto" }}>
      ...
      {/* ── Fórmulas asociadas ── */}
      <div style={{ background: "#f4f8fb", borderRadius: 10, padding: "14px 16px", marginBottom: 20, border: "1px solid #e0eaf2" }}>
        {/* Header */}
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", background: "#0f2944", borderRadius: 7, padding: "9px 14px", marginBottom: 12 }}>
          <span style={{ fontWeight: 700, fontSize: 13, letterSpacing: "0.12em", textTransform: "uppercase", color: "#fff" }}>
            🧮 Fórmulas
          </span>
          {cargandoSlots && <span style={{ fontSize: 11, color: "#7ab2d4", fontStyle: "italic" }}>⏳ Calculando...</span>}
          {!cargandoSlots && slotsFormulas.length > 0 && (
            <span style={{ fontSize: 11, color: "#7ab2d4", letterSpacing: "0.08em" }}>
        ...
        {!cargandoSlots && slotsFormulas.length === 0 && (
          <p style={{ fontSize: 12, color: "#8aabb8", fontStyle: "italic", padding: "6px 2px" }}>
        ...
        {slotsFormulas.map((slot, i) => {
          ...
          return (
          <div key={slotNum} style={{ background: "#fff", borderRadius: 7, border: "1px solid #e0eaf2", marginBottom: 7, padding: "10px 12px" }}>
            {/* Encabezado fórmula */}
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8 }}>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontWeight: 700, fontSize: 13, color: "#0f2944", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
                  {slot.nombre}
                </div>
                {slot.codform && (
                  <div style={{ fontSize: 10, color: "#4a8ab5", fontFamily: "monospace", marginTop: 2, letterSpacing: "0.04em" }}>
                    #{slot.codform}
                  </div>
                )}
              </div>
              <div style={{ fontWeight: 700, fontSize: 16, color: slot.resultado > 0 ? "#0f2944" : "#b0c8d8", minWidth: 100, textAlign: "right", flexShrink: 0 }}>
            ...
            {slot.expresion && (
              <div style={{ marginTop: 7, padding: "4px 8px", background: "#eaf3fb", border: "1px solid #b8d6ef", borderRadius: 5, fontSize: 11, fontFamily: "monospace", color: "#1a4a70", wordBreak: "break-all", lineHeight: 1.5 }}>
                <span style={{ fontSize: 9, fontWeight: 700, letterSpacing: "0.1em", color: "#4a8ab5", textTransform: "uppercase", fontFamily: "sans-serif", marginRight: 4 }}>expr ·</span>
                {slot.expresion}
              </div>
            )}

            {/* ── Mano de obra ── */}
            <div style={{ marginTop: 10, padding: "10px 12px", background: "#fffbeb", border: `1.5px solid ${esError ? "#f87171" : moVal > 0 ? "#f59e0b" : "#fde68a"}`, borderRadius: 7 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 6 }}>
                <span style={{ fontSize: 10, fontWeight: 800, color: "#92400e", letterSpacing: "0.1em", textTransform: "uppercase" }}>🛠 Mano de obra</span>
                <span style={{ fontSize: 9, color: "#b45309", fontStyle: "italic" }}>
                  — número fijo o expresión (ej: <code style={{ fontFamily: "monospace", background: "#fef3c7", padding: "0 3px", borderRadius: 3 }}>ancho * alto * 500</code>)
                </span>
              </div>
              ...
              <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
                ...
                <input
                  ...
                  style={{
                    ...
                    outline: "none", background: "#fff", color: "#78350f",
                    transition: "border-color 0.2s",
                  }}
                  onFocus={e => e.target.style.borderColor = "#f59e0b"}
                  onBlur={e => e.target.style.borderColor = esError ? "#f87171" : "#fcd34d"}
                />
                {/* Resultado evaluado */}
                <div style={{
                  minWidth: 90, textAlign: "right", fontWeight: 800,
                  fontSize: 14, color: esError ? "#ef4444" : moVal > 0 ? "#b45309" : "#d1d5db",
                  flexShrink: 0,
                }}>
              ...
              {exprVal.trim() !== "" && !esError && moVal === 0 && (
                <div style={{ fontSize: 10, color: "#b45309", marginTop: 4, fontStyle: "italic" }}>
                  Resultado 0 — variables disponibles: ancho, alto, profundo, precio_material, precio_lateral_izq/der, precio_techo, precio_base, cajon1_ancho, cajon1_alto, cajon1_cantidad…
                </div>
              )}
              {esError && (
        ...
        {slotsFormulas.length > 0 && (
          <div style={{ marginTop: 8, padding: "10px 14px", background: "#f0f7f0", border: "1px solid #b8dfc8", borderRadius: 7 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
              <span style={{ fontWeight: 700, fontSize: 11, letterSpacing: "0.14em", color: "#16a34a", textTransform: "uppercase" }}>
                📈 MARGEN (%)
              </span>
              {margenBD !== null && (
                <span style={{
                  fontSize: 10, fontWeight: 600,
                  background: Number(margen) !== margenBD ? "#fff3cd" : "#eaf3fb",
                ...
                onClick={() => Number(margen) !== margenBD && setMargen(margenBD)}
                title={Number(margen) !== margenBD ? `Restaurar BD (${margenBD}%)` : "Valor de BD"}>
                  {Number(margen) !== margenBD ? `⚠️ BD: ${margenBD}% — restaurar` : `📊 BD: ${margenBD}%`}
                </span>
              )}
            </div>
            <input
              type="number" min="0" step="0.5" value={margen}
              onChange={e => setMargen(Number(e.target.value))}
              style={{ width: "100%", padding: "9px 12px", borderRadius: 6, border: "1.5px solid #b8cfe0", fontSize: 14, fontFamily: "inherit", outline: "none", background: "#fff", color: "#0a3a5c", marginBottom: 10 }}
            />
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 4, padding: "8px 12px", background: "linear-gradient(90deg,#1a3a5c,#0f2944)", borderRadius: 5 }}>
              <span style={{ fontWeight: 700, fontSize: 11, letterSpacing: "0.14em", color: "#7ab2d4", textTransform: "uppercase" }}>Subtotal</span>
              <span style={{ fontWeight: 700, fontSize: 20, color: "#60b4f0" }}>{formatPeso(baseMargen + totalMargen)}</span>
            </div>
          </div>
        )}
      </div>
      ...
      <div style={{ background: "#fff", border: "1.5px solid #b8cfe0", borderRadius: 12, padding: "22px 24px", marginBottom: 20 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
          <div style={{ fontSize: 12, fontWeight: 700, color: "#0a3a5c", letterSpacing: 2, textTransform: "uppercase" }}>
            🚚 Colocación
          </div>
          {colocacionBD !== null && (
            <span style={{
              ...
              border: "1px solid #b8d6ef", borderRadius: 4, padding: "1px 7px",
            }}>
              📊 BD:{colocacionBD.valor !== null ? ` $${colocacionBD.valor.toLocaleString("es-AR")}` : ""}
              {colocacionBD.valor !== null && colocacionBD.porcentaje !== null ? " / " : ""}
              {colocacionBD.porcentaje !== null ? ` ${colocacionBD.porcentaje}%` : ""}
            </span>
          )}
        </div>
          ...
          {[
            ...
            <button key={key} type="button"
              ...
              style={{
                ...
                border: colocacionModo === key ? "2px solid #2563eb" : "1.5px solid #b8cfe0",
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
          ...
          <div style={{
            ...
            <div style={{ fontSize: 10, fontWeight: 700, color: colocacionModo === "valor" ? "#2563eb" : "#94a3b8", letterSpacing: "0.1em", textTransform: "uppercase", marginBottom: 6 }}>
              Valor ($)
              {colocacionBD?.valor !== null && colocacionBD?.valor !== undefined && (
                <span
                  onClick={() => { setColocacionValor(colocacionBD.valor); setColocacionModo("valor"); }}
                  style={{ marginLeft: 6, cursor: "pointer", color: "#2563eb", fontWeight: 600 }}
                  title="Restaurar BD">↺ BD</span>
              )}
            </div>
            <input type="number" min="0" step="100" value={colocacionValor}
              onChange={e => { setColocacionValor(Number(e.target.value)); setColocacionModo("valor"); }}
              style={{ width: "100%", padding: "8px 10px", borderRadius: 6, border: "1.5px solid #b8cfe0", fontSize: 14, fontFamily: "inherit", outline: "none", background: "#fff", color: "#0a3a5c" }}
            />
            <div style={{ fontSize: 11, color: colocacionModo === "valor" ? "#2563eb" : "#94a3b8", marginTop: 6, fontWeight: 600 }}>
          ...
          <div style={{
            ...
            background: colocacionModo === "porcentaje" ? "#f5f3ff" : "#f8fafc",
            opacity: colocacionModo === "porcentaje" ? 1 : 0.5,
          }}>
            <div style={{ fontSize: 10, fontWeight: 700, color: colocacionModo === "porcentaje" ? "#7c3aed" : "#94a3b8", letterSpacing: "0.1em", textTransform: "uppercase", marginBottom: 6 }}>
              % del subtotal
              {colocacionBD?.porcentaje !== null && colocacionBD?.porcentaje !== undefined && (
                <span
                  onClick={() => { setColocacionPct(colocacionBD.porcentaje); setColocacionModo("porcentaje"); }}
                  style={{ marginLeft: 6, cursor: "pointer", color: "#7c3aed", fontWeight: 600 }}
                  title="Restaurar BD">↺ BD</span>
              )}
            </div>
            <input type="number" min="0" step="0.5" value={colocacionPct}
              onChange={e => { setColocacionPct(Number(e.target.value)); setColocacionModo("porcentaje"); }}
              style={{ width: "100%", padding: "8px 10px", borderRadius: 6, border: "1.5px solid #b8cfe0", fontSize: 14, fontFamily: "inherit", outline: "none", background: "#fff", color: "#0a3a5c" }}
            />
            <div style={{ fontSize: 11, color: colocacionModo === "porcentaje" ? "#7c3aed" : "#94a3b8", marginTop: 6, fontWeight: 600 }}>
      ...
      {/* Breakdown final */}
      {slotsFormulas.length > 0 && (
        <div style={{ background: "#f4f8fb", borderRadius: 8, overflow: "hidden", marginBottom: 24 }}>
          <div style={{ display: "flex", justifyContent: "space-between", padding: "12px 16px", fontSize: 14, color: "#4a6a80", borderBottom: "1px solid #e0eaf2", fontWeight: 600 }}>
            <span>Fórmulas ({slotsFormulas.length} ítems)</span>
            <span>{formatPeso(totalSlots)}</span>
          </div>
          {Number(margen) > 0 && (
            <div style={{ display: "flex", justifyContent: "space-between", padding: "12px 16px", fontSize: 14, color: "#4a6a80", borderBottom: "1px solid #e0eaf2" }}>
              <span>Margen ({margen}%)</span>
              <span>{formatPeso(totalMargen)}</span>
            </div>
          )}
          {totalManoObra > 0 && (
            <div style={{ display: "flex", justifyContent: "space-between", padding: "12px 16px", fontSize: 14, color: "#92400e", borderBottom: "1px solid #e0eaf2", background: "#fffbeb" }}>
              <span>🛠 Mano de obra</span>
              <span style={{ fontWeight: 700 }}>{formatPeso(totalManoObra)}</span>
            </div>
          )}
          {totalColocacion > 0 && (
            <div style={{ display: "flex", justifyContent: "space-between", padding: "12px 16px", fontSize: 14, color: "#4a6a80", borderBottom: "1px solid #e0eaf2" }}>
              <span>Colocación {colocacionModo === "porcentaje" ? `(${colocacionPct}%)` : ""}</span>
              <span>{formatPeso(totalColocacion)}</span>
            </div>
          )}
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "14px 16px", background: "#0f2944", color: "#fff" }}>
            <span style={{ fontWeight: 700, fontSize: 13, letterSpacing: "0.14em" }}>TOTAL</span>
            <span style={{ fontWeight: 700, fontSize: 22 }}>{formatPeso(total)}</span>
          </div>
        </div>
      )}
