...
export default function PresupuestoDespensero({ modelo: modeloRaw, onVolver }) {
  ...
  useEffect(() => {
    ...
    fetch(
      ...
      .catch((err) => {
        console.error("[Despensero] Error cargando margen:", err);
        setMargenBD(null);
      });
  }, [modelo?.codart]);
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([mats, bis, her]) => {
        ...
        if (mats.length > 0)
          console.log("[Despensero] Ejemplo placa:", mats[0]);
      })
