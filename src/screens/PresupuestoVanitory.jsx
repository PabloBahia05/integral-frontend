...
export default function PresupuestoVanitory({ modelo: modeloRaw, onVolver, numeroPres, cliente, codcliente, revision, onGuardado }) {
  ...
  useEffect(() => {
    ...
    fetch(`${API}/colocacion/buscar?codart=${encodeURIComponent(modelo.codart)}`)
      ...
      .then(d => {
        ...
        setColocacionBD(bd);
        if (bd.valor !== null)           { setColocacionModo("valor");      setColocacionValor(bd.valor); setForm(prev => ({ ...prev, colocacion: bd.valor })); }
        else if (bd.porcentaje !== null) { setColocacionModo("porcentaje"); setColocacionPct(bd.porcentaje); setForm(prev => ({ ...prev, colocacion: 0 })); }
      })
  ...
  useEffect(() => {
    ...
    setTotalSlots(Math.round(suma * 100) / 100);
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [slotsFormulas.length, form.ancho, form.alto, form.profundo, form.cantidad, form.colocacion, form.materialPrecio, form.materialBlancoPrecio, form.lateralDer, form.lateralIzq, form.base, form.techo, cargandoInsumos]);
              ...
              <div className="field">
                <span className="label-text" style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                  ...
                  {colocacionBD !== null && (
                    <span style={{
                      ...
                      📊 BD:{colocacionBD.valor != null ? ` $${Number(colocacionBD.valor).toLocaleString("es-AR")}` : ""}
                  ...
                  {[
                    ...
                    <button key={key} type="button"
                      ...
                      style={{
                        ...
                        border: colocacionModo === key ? "2px solid #2d7fc1" : "1.5px solid #d0dde8",
                        background: colocacionModo === key ? "#eaf3fb" : "#f8fafc",
                ...
                <div style={{display:"grid", gridTemplateColumns:"1fr 1fr", gap:10}}>
                  ...
                  <div style={{
                    ...
                    background: colocacionModo==="valor" ? "#eaf3fb" : "#f8fafc",
                    opacity: colocacionModo==="valor" ? 1 : 0.5,
                  }}>
                    <div style={{fontSize:10, fontWeight:700, color: colocacionModo==="valor" ? "#2d7fc1" : "#94a3b8", letterSpacing:"0.1em", textTransform:"uppercase", marginBottom:5}}>
                      Valor ($)
                      {colocacionBD?.valor != null && (
                        <span onClick={() => { setColocacionValor(colocacionBD.valor); setColocacionModo("valor"); }}
                          style={{marginLeft:6, cursor:"pointer", color:"#2d7fc1"}} title="Restaurar BD">↺ BD</span>
                      )}
                    </div>
                    ...
                    />
                    <div style={{fontSize:11, color: colocacionModo==="valor" ? "#2d7fc1" : "#94a3b8", marginTop:5, fontWeight:600}}>
                  ...
                  <div style={{
                    ...
                    <div style={{fontSize:10, fontWeight:700, color: colocacionModo==="porcentaje" ? "#7c3aed" : "#94a3b8", letterSpacing:"0.1em", textTransform:"uppercase", marginBottom:5}}>
                      % del subtotal
                      {colocacionBD?.porcentaje != null && (
                        <span onClick={() => { setColocacionPct(colocacionBD.porcentaje); setColocacionModo("porcentaje"); }}
                          style={{marginLeft:6, cursor:"pointer", color:"#7c3aed"}} title="Restaurar BD">↺ BD</span>
                      )}
                    </div>
                    ...
                    />
                    <div style={{fontSize:11, color: colocacionModo==="porcentaje" ? "#7c3aed" : "#94a3b8", marginTop:5, fontWeight:600}}>
